...
import { stripAnsi, truncateToWidth, visibleWidth, WelcomeHeader } from "./welcome.ts";
...
function header(overrides: Partial<ConstructorParameters<typeof WelcomeHeader>[0]> = {}) {
...
describe("wide welcome box", () => {
  test("renders OMP geometry, gradient logo, Pi tips, and ordered sections", () => {
    ...
    expect(rendered.filter(line => line.includes("\x1b[38;2;"))).not.toHaveLength(0);
    expect(sectionIndex(lines, "Tips")).toBeLessThan(sectionIndex(lines, "Extensions"));
    expect(sectionIndex(lines, "Extensions")).toBeLessThan(sectionIndex(lines, "Recent sessions"));
    expect(lines.filter(line => line.includes("for commands") || line.includes("to run bash") || line.includes("drop files to attach"))).toHaveLength(4);
    expect(lines.filter(line => line.includes("just now") || line.includes("2m ago") || line.includes("1h ago") || line.includes("3d ago"))).toHaveLength(4);
  ...
  test("omits model details, keeps list entries on separate rows, and uses one-row vertical padding", () => {
    ...
    const lines = rawLines(header({ extensions, recentSessions: sessions }), 102);
    const bottomIndex = lines.findIndex(line => line.startsWith("╰"));
    const boxRows = lines.slice(1, bottomIndex);
    expect(lines.some(line => line.includes("Claude Sonnet") || line.includes("Anthropic"))).toBe(false);
    expect(boxRows.filter(line => extensions.some(extension => line.includes(extension.name)))).toHaveLength(extensions.length);
    expect(boxRows.filter(line => sessions.some(session => line.includes(session.name)))).toHaveLength(sessions.length);
  ...
  test("preserves three extension names and overflow when the wide header must scroll", () => {
    const extensions = Array.from({ length: 6 }, (_, index) => ({ name: `extension-${index + 1}.ts`, scope: "user" as const }));
    const lines = rawLines(header({ extensions, terminalRows: () => 17 }), 102);

    expect(lines).toHaveLength(19);
    expect(lines.filter(line => line.includes("extension-")).map(line => line.match(/extension-\d+\.ts/)?.[0])).toEqual(["extension-1.ts", "extension-2.ts", "extension-3.ts"]);
...
describe("narrow welcome box", () => {
  test("stacks left content then all named sections in one rounded box", () => {
    ...
    expect(sectionIndex(lines, "Welcome back!")).toBeLessThan(sectionIndex(lines, "Tips"));
    expect(sectionIndex(lines, "Tips")).toBeLessThan(sectionIndex(lines, "Extensions"));
    expect(sectionIndex(lines, "Extensions")).toBeLessThan(sectionIndex(lines, "Recent sessions"));
  ...
  test("shows empty section rows and truncates values before trailing metadata", () => {
    const lines = rawLines(header({
      extensions: [{ name: "an-exceptionally-long-extension-name-that-needs-truncation.ts", scope: "user" }],
      recentSessions: [{ name: "A session name that is intentionally too long to fit in this narrow row", timeAgo: "6d ago" }],
    }), 32);
    ...
    expect(lines.filter(line => line.startsWith("│")).every(line => visibleWidth(line) === 30)).toBe(true);

    const empty = rawLines(header(), 32);
    expect(empty.some(line => line.includes("No extensions"))).toBe(true);
    expect(empty.some(line => line.includes("No recent sessions"))).toBe(true);
  });
});
...
describe("terminal-cell-safe rendering", () => {
  ...
  test("keeps every box row within its terminal-cell budget for wide Unicode", () => {
    const component = header({
      ...
      recentSessions: [{ name: "会話e\u0301👩‍💻-長い名前", timeAgo: "just now" }],
      selectedTip: "Use 👩‍💻 and 界界 safely in a terminal-cell-aware tip.",
    });
    const lines = component.render(102);
    const box = lines.slice(0, lines.findIndex(line => stripAnsi(line).startsWith("╰")) + 1);

    expect(box.every(line => visibleWidth(line) === 100)).toBe(true);
    expect(lines.slice(box.length).every(line => visibleWidth(line) <= 100)).toBe(true);
    component.dispose();
  });
});
